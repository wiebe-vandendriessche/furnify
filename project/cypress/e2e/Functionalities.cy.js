...
describe('First test',()=>{
  ...
  it("Adding and deleting obstructions", ()=>{
    ...
    for(let i=0; i<amount; i++){
      cy.get('[datatest-id="btn-space-aspect-window"]').click();
    }
    cy.get('[datatest-id="btn-space-aspect-door"]').click();

    //amount of window-obstructions should equal amount of obstructions added
    cy.get('[datatest-id="btn-obstacle-expand-window"]').should("have.length", amount);
    //should find door-obstruction and scroll it into view
    cy.get('[datatest-id="btn-obstacle-expand-door"]').scrollIntoView();

    //deleting obstructions
    for(let i=0; i<Math.floor(amount/2); i++){
      cy.get('[datatest-id="btn-obstacle-delete-window"]').eq(i).scrollIntoView();
      ...
      cy.get('[datatest-id="btn-obstacle-delete-window"]').eq(i).click({force: true});
    }
    //checking amount of not deleted obstructions is correct
    cy.get('[datatest-id="btn-obstacle-expand-window"]').should("have.length", amount-Math.floor(amount/2));
    cy.get('[datatest-id="btn-obstacle-expand-door"]').scrollIntoView();
   ...
   it("Adding obstruction, changing data of that obstruction and making sure data didn't disappear", ()=>{
      ...
      cy.get('[datatest-id="btn-space-aspect-window"]').should("be.visible").click();
      //when clicking the button a window-obstacle should be added, now we want to expand it
      cy.get('[datatest-id="btn-obstacle-expand-window"]').click();
      //Entering dimensions
      ...
      cy.get('[datatest-id="input-obst-'+params.type+'-length"').should('have.value', params.Length);
      cy.get('[datatest-id="input-obst-'+params.type+'-width"').should('have.value', params.Width);
      cy.get('[datatest-id="input-obst-'+params.type+'-height"').should('have.value', params.Height);
   ...
   it("Questionnaire functional- selecting room and needed functions", ()=>{
       ...
       cy.get('[datatest-id="btn-nav-sidebar-next"]').should("be.visible").click();

       //Clicking on button guestroom
       cy.get('input#guestroom').should("not.be.checked");
       cy.get('[datatest-id="btn-func-room-guestroom"]').should("be.visible").click();
       cy.get('input#guestroom').should("be.checked");

       //Matress options should not be visible when bed isn't clicked
       cy.get('[datatest-id="div-func-options-mattress"]').should("not.be.visible");
       //Selecting option bed
       cy.get('[datatest-id="btn-func-bed"]').should("be.visible").click();
       cy.get('input#none').should("be.checked");
       cy.get('[datatest-id="btn-func-bed-medium"]').click();
       ...
       cy.get('[datatest-id="btn-nav-sidebar-next"]').should("be.visible").click();
       cy.get('[datatest-id="btn-nav-sidebar-previous"]').should("be.visible").click();
       cy.get('input#medium').should("be.checked");
       cy.get('input#guestroom').should("be.checked");
   });
   ...
   it("Questionnaire specifications- selecting options and inserting text into text-area", ()=>{
       ...
       cy.get('[datatest-id="btn-specs-preferences-partition_wall"]').click();
       cy.get('input#partition').should("be.checked");
       cy.get('[datatest-id="btn-specs-material-oak"]').click();
       ...
       cy.get('[datatest-id="btn-specs-color-white"]').click();
       cy.get('input#colWhite').should("be.checked");
       cy.get('input#colBlack').should("not.be.checked");
       cy.get('[datatest-id="area-specs-requirements"]').clear().type(text);
       ...
       cy.get('input#colWhite').should("be.checked");
       cy.get('input#colBlack').should("not.be.checked");
       cy.get('[datatest-id="area-specs-requirements"]').should("have.text", text);
   });
   ...
   it("Questionnaire contact- ", ()=>{
       ...
       cy.get('[datatest-id="btn-nav-sidebar-next"]').should("not.be.visible");

       //Inserting text into the inputs
       cy.get('[datatest-id="input-contact-firstname"]').clear().type(contact.firstname);
       cy.get('[datatest-id="input-contact-lastname"]').clear().type(contact.lastname);
       cy.get('[datatest-id="input-contact-email"]').clear().type(contact.email);
       ...
       cy.get('[datatest-id="input-contact-firstname"]').should("have.value", contact.firstname);
       cy.get('[datatest-id="input-contact-lastname"]').should("have.value", contact.lastname);
       cy.get('[datatest-id="input-contact-email"]').should("have.value", contact.email);
       cy.get('input[type="tel"]').assertInputValueWithoutSpaces(contact.phoneInput).should("be.true");
   })
